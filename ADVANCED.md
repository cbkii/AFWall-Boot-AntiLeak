# AFWall Boot AntiLeak — Advanced Documentation (v2.2.2)

> For the concise user guide, see [README.md](README.md).

This document provides comprehensive technical details for AFWall Boot AntiLeak v2.2.2.
It covers the layered anti-leak design, connectivity coverage matrix, AFWall interaction,
lower-layer suppression subsystem, verification commands, recovery procedures, and developer
reference.

For the concise user guide (install, basic config, basic recovery), see [README.md](README.md).
...
2. Select `AFWall-Boot-AntiLeak-v2.2.2.zip`.
...
```
[2026-03-08 04:01:00] post-fs-data: start (module=AFWall-Boot-AntiLeak v2.2.2)
...
### v2.2.2 — Final completion and release hardening

- **CI fix**: added `bin/lowlevel.sh` to shellcheck step in `.github/workflows/ci.yml`;
  added `ADVANCED.md` to the `paths-ignore` list.
- **README split**: `README.md` is now a concise user guide; `ADVANCED.md` holds all
  technical/advanced detail (this file).
- **Version bump**: v2.2.0 → v2.2.2 / versionCode 220 → 222.
- **update.json**: `changelog` URL updated to point to `ADVANCED.md`.
- All version strings updated consistently across `module.prop`, `update.json`,
  `bin/common.sh`, `bin/lowlevel.sh`, `post-fs-data.sh`.
...
Output:
- `dist/AFWall-Boot-AntiLeak-v2.2.2.zip` — Magisk-installable module ZIP
...
tools/validate-zip.sh dist/AFWall-Boot-AntiLeak-v2.2.2.zip
```
...
1. Push a version tag (e.g. `v2.2.2`) matching the value in `module.prop`.
...
3. **Specific version**: `https://github.com/cbkii/AFWall-Boot-AntiLeak/releases/download/v2.2.2/AFWall-Boot-AntiLeak.zip`
...
```json
{
  ...
  "versionCode": 222,
  "zipUrl": "https://github.com/cbkii/AFWall-Boot-AntiLeak/releases/latest/download/AFWall-Boot-AntiLeak.zip",
  "changelog": "https://raw.githubusercontent.com/cbkii/AFWall-Boot-AntiLeak/master/ADVANCED.md"
}
```
...
unzip -l dist/AFWall-Boot-AntiLeak-v2.2.2.zip
```
