...
def main() -> int:
    ...
    replace("bin/lowlevel.sh", [(r"(# AFWall Boot AntiLeak )v\d+\.\d+\.\d+", rf"\g<1>{version}"), (r"derived from v\d+\.\d+\.\d+ VPN_LOCKDOWN_MODE", f"derived from {version} VPN_LOCKDOWN_MODE")])
    ...
    replace("config.sh", [(r"(# AFWall Boot AntiLeak )v\d+\.\d+\.\d+", rf"\g<1>{version}"), (r"Breaking-change note: v\d+\.\d+\.\d+", f"Breaking-change note: {version}")])
    replace("common/install.sh", [(r"AFWall Boot AntiLeak\s+v\d+\.\d+\.\d+", f"AFWall Boot AntiLeak  {version}"), (r"\[v\d+\.\d+\.\d+ BREAKING CONFIG CLEANUP\]", f"[{version} BREAKING CONFIG CLEANUP]")])
